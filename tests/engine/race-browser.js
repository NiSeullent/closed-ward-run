async (page) => {
 ...
 await f.getByText(/상대 .*m 차이/).waitFor({timeout:10000});
 let fail=true;
 ...
 await f.locator('summary').click();await g.locator('summary').click();
 await g.getByRole('button',{name:'랭킹 새로고침',exact:true}).click();
 await f.locator('li').filter({hasText:'player1 · 640m'}).waitFor();
 await g.locator('li').filter({hasText:'player1 · 640m'}).waitFor();
